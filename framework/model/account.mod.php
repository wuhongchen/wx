...
function uni_create_permission($uid, $type = 1) {
	...
	$uniacidnum = count($uniacount);
		...
		$acidnum = 0;
		if(!empty($uniacid)) {
			$uniacidstr = implode(',', $uniacid);
			$acidnum = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('account') . ' WHERE uniacid IN (' . $uniacidstr . ')');
		}
		if($acidnum >= $groupdata['maxsubaccount']) {
...
function uni_modules($enabledOnly = true) {
	...
	foreach($modules as $k => $v) {
		if($v['issolution'] && $v['target'] != $uniacid) {
			unset($modules[$k]);
		}
	}
	if (!empty($modules)) {
...
function account_weixin_basic($username) {
	...
	if(strexists($response['content'], '订阅号')) {
		if(strexists($response['content'], '微信认证')) {
			$info['level'] = 1;
			$is_key_secret = 1;
		}
