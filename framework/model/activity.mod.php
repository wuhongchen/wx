...
function activity_coupon_owned($uid, $filter = array(), $pindex = 1, $psize = 10) {
	...
	if($psize > 0) {
		$data = pdo_fetchall("SELECT r.*,c.couponid,c.title,c.type,c.description,c.discount,c.starttime,c.endtime,c.thumb FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 1 AND r.uid = :uid ".$condition." ORDER BY r.recid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array(':uid' => $uid));
	} else {
		$data = pdo_fetchall("SELECT r.*,c.couponid,c.title,c.type,c.description,c.discount,c.starttime,c.endtime,c.thumb FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 1 AND r.uid = :uid ".$condition.' ORDER BY r.recid DESC ',array(':uid' => $uid));
	}
...
function activity_token_owned($uid, $filter = array(), $pindex = 10, $psize = 0) {
	...
	if($psize > 0) {
		$data = pdo_fetchall("SELECT r.*,c.couponid,c.condition,c.title,c.discount,c.type,c.starttime,c.endtime FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 2 AND r.uid = :uid ".$condition." ORDER BY r.recid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array(':uid' => $uid));
	} else {
		$data = pdo_fetchall("SELECT r.*,c.couponid,c.condition,c.title,c.discount,c.type,c.starttime,c.endtime FROM ".tablename('activity_coupon_record')." AS r LEFT JOIN ".tablename('activity_coupon')." AS c ON r.couponid = c.couponid WHERE c.type = 2 AND r.uid = :uid ".$condition . ' ORDER BY r.recid DESC ',array(':uid' => $uid));
	}
	return array('total' => $total,'data' => $data);
}
