...
if($do == 'clerk') {
		...
		if (!empty($_GPC['title'])) {	
			foreach ($_GPC['title'] as $index => $row) {
				...
				if (!empty($_GPC['password'][$index])) {
					$data['password'] = user_hash($_GPC['password'][$index], '');
				}
				if(!empty($data['name'])) {
		...
		if (!empty($_GPC['title-new'])) {
			foreach ($_GPC['title-new'] as $index => $row) {
				$data = array(
						...
						'password' => user_hash($_GPC['password-new'][$index], ''),
				);
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('activity_coupon_password')." WHERE uniacid = :uniacid", array(':uniacid' => $_W['weid']));
