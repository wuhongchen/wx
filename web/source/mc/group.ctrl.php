...
if($do == 'display') {
	...
	$list = pdo_fetchall("SELECT * FROM ".tablename('mc_groups')." WHERE uniacid = :weid ORDER BY orderlist ASC,groupid ASC", array(':weid' => $_W['weid']));
}
...
if($do == 'delete') {
	...
	$groupid = intval($_GPC['id']);
	pdo_query("DELETE FROM " . tablename('mc_groups') . " WHERE uniacid = :uniacid AND groupid = :groupid",array(':uniacid' => $_W['weid'],':groupid' => $groupid));
	message('用户组删除成功！', url('mc/group/display'), 'success');
}

if($do == 'set') {
	$groupid = intval($_GPC['id']);
	pdo_update('mc_groups', array('isdefault' => 0), array('uniacid' => $_W['weid']));
	pdo_update('mc_groups', array('isdefault' => 1), array('uniacid' => $_W['weid'],'groupid' => $groupid));
