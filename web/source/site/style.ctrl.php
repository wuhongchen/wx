...
if($do == 'default') {
	...
	$style = pdo_fetch("SELECT * FROM ".tablename('site_styles')." WHERE id = :styleid", array(':styleid' => $styleid));
	if(empty($style)) {
	...
	message('默认模板更新成功！', url('site/style/styles'), 'success');
}
...
if ($do == 'build') {
	...
	message('风格创建成功，进入进入“设计风格”界面。', url('site/style/designer', array('templateid' => $template['id'], 'styleid' => $id)), 'success');
}
