...
$title = array('home'=>'微站首页导航图标','profile'=>'个人中心功能条目','shortcut'=>'快捷菜单');
$_W['page']['title'] = $title[$type['name']];
...
if($do == 'post') {
	...
	if(checksubmit('submit')) {
		...
		message('导航更新成功！', url('site/nav', array('do'=>$type['name'])), 'success');
	}
...
if($do == 'display') {
	...
	if($_W['ispost']) {
		...
		if(is_array($set)) {
			if(isset($set['multiid'])) {
				$sql = 'SELECT * FROM ' . tablename('site_nav') . ' WHERE `position`=:position AND `uniacid`=:uniacid AND `module`=:module AND `url`=:url AND `multiid`=:multiid';
	...
	if(!empty($navs)) {
		foreach($navs as $nav) {
			...
			$multi_title = pdo_fetchcolumn('SELECT title FROM ' . tablename('site_multi') . ' WHERE uniacid = :uniacid AND id = :id', array(':uniacid' => $_W['uniacid'], ':id' => $nav['multiid']));
			$navigations[] = array(
				'id' => $nav['id'],
				'module' => $nav['module'],
				'title' => $nav['name'],
