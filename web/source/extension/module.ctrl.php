...
$dos = array('installed', 'check', 'prepared', 'install', 'upgrade', 'uninstall', 'designer', 'permission', 'batch-install');
		...
		foreach ($modules as $mid => $module) {
			...
			if(is_array($manifest) && ver_compare($module['version'], $manifest['application']['version']) == '-1') {
				$modules[$mid]['upgrade'] = true;
						...
						if(in_array($module['name'], array('basic', 'news', 'music', 'userapi', 'welcome', 'cover', 'default'))) {
	...
	if(checksubmit() && $available[$_GPC['method']]) {
		...
		if($_GPC['method'] == 'create') {
			...
			if($m['application']['setting']) {
				f_write("{$mRoot}/template/settings.html", "{template 'common/header'}\r\n这里定义页面内容\r\n{template 'common/footer'}");
			}
			if($m['icon']) {
