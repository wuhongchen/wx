...
if($do == 'display') {
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM '. tablename('activity_coupon'). ' WHERE uniacid = :uniacid AND type = :type' , array(':uniacid' => $_W['uniacid'], ':type' => 2));
	$pindex = max(1, intval($_GPC['page']));
	$psize = 10;
	$lists = pdo_fetchall('SELECT couponid,title,thumb,type,credittype,credit,endtime,description FROM ' . tablename('activity_coupon') . ' WHERE uniacid = :uniacid AND type = :type ORDER BY endtime ASC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, array(':uniacid' => $_W['uniacid'], ':type' => 2));
...
if($do == 'use') {
	$id = intval($_GPC['id']);
	$data = activity_token_owned($_W['member']['uid'], array('couponid' => $id));
	$data = $data['data'][0];
	if(checksubmit('submit')) {
		...
		if(!empty($clerk)) {
			$status = activity_token_use($_W['member']['uid'], $id);
			if (!is_error($status)) {
				message('代金券使用成功！', url('activity/token/mine', array('type' => $_GPC['type'])), 'success');
			}
			message('代金券使用失败！', url('activity/token/mine', array('type' => $_GPC['type'])), 'error');
		}
